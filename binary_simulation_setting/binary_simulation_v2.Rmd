...
```
```{r function for optimal coefficients for desired prop, eval = FALSE, include = FALSE}
...
covariate_coef <- function(desired_prop, cov_df) {
   ...
   # Is this deciding treatment effect? 
   ...
   while (abs(p_A[i] - 0.14) > tol) {
     ...
     if (abs(p_A[i] - 0.14) < tol) { ##### is this the treatment rate hard coded????? 
       ###### might want to edit herre1!!!1!! 
...
```
```{r old data gen function, eval = FALSE, include = FALSE}
...
generate_no_boot_data <- function(n, size = 5000, seeds = seed_vec) {
  ...
  pb <- progress_bar$new(format = "generating data... [:bar]", total = n)
    ...
    pre_data <- defData(pre_data, varname = "A", 
                        ...
                        # L3 should not affect treatment status
                        # why did you choose -1.815? 
                        # this should be log(desired_treat_rate / (1- desired_treat_rate))
                        # thus the rate is about 0.14% 
                    dist = "binary", link = "logit")
    
    pre_data <- defData(pre_data, varname = "Y", 
                        formula = "0 + 0.767*A + 1.39*L2 -0.7*L3 + beta_error" ,
                        # why are we hard coding the coef here but not in treatment status?
                    dist = "binary", link = "logit")
