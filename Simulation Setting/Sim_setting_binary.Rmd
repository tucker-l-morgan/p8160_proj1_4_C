...
data_gen <- function(desired_prop) {
   ...
   coef_L1 <- sample(df$L2, 1000, replace = TRUE)
   
   coef_L2 <- sample(df$L2, 1000, replace = TRUE)
   
   coef_L3 <- sample(df$L2, 1000, replace = TRUE)
   
   A_logit <- vector(mode = "list",length = length(coef_L2))
   p_A <-  vector(mode = "numeric",length = length(coef_L2))
   
   u <- alpha_0 + coef_L2[1]*df$L1 + coef_L2[1]*df$L2 - coef_L3[1]*df$L3
...
pre_data <- defData(pre_data, varname = "A", 
                    formula = "-1.815 + 0.38*L1 + 0.167*L2 + 0.19*L3", 
...
mean(df$A) #proportion of getting treated
```
