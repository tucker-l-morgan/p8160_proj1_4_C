...
boot_result <-
  ...
  summarize(ATE = mean(estimate),
            sd_ATE = sd(estimate),
            perc_2.5 = quantile(estimate, probs = 0.025),
            perc_97.5 = quantile(estimate, probs = 0.975))
...
cont_simple_df <- 
  boot_result %>% 
  mutate(ci_lower = ATE - qnorm(0.975)*sd(ATE),
         ci_upper = ATE + qnorm(0.975)*sd(ATE),
         covered = case_when(
           ci_lower <= beta1 & ci_upper >= beta1 ~ 1,
                                            TRUE ~ 0
         ),
