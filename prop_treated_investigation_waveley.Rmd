...
data_gen <- function(N, desired_prop) {
  ...
  # get the desired treatment rate 
  alpha_0_treat = log(desired_prop/(1 - desired_prop))
  
  # Creating the treatment assignment, setting alpha_1=0.4, alpha_2 = 0.5, no error term
  A_logit <- alpha_0_treat + 0.4*df$L1 + 0.5*df$L2
  p_A     <- exp(A_logit)/(1 + exp(A_logit))
  A       <- rbinom(N, 1, p_A)
  
  ### sanity check. mean(A) should be close to desired_prop
  # mean(A)
